    ...
    def handle(self, *args, **options):
        ...
        for data_dict in models.get("Contributor", []):
            c, created = Contributor.objects.get_or_create(
                first_name=data_dict["contributor_first_names"],
        ...
        for data_dict in models.get("Bookcontributor", []):
            ...
            if created:
                print(
                    'Created Bookcontributor "{}" -> "{}"'.format(
                        contributor.email, book.title
                    )
                )

        for data_dict in models.get("Review", []):
            creator, created = User.objects.get_or_create(
                email=data_dict["review_creator"], username=data_dict["review_creator"]
            )
